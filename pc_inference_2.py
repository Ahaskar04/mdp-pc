...
# --- GLOBAL STATE ---
# ACTIVE_SNAP_ID is now primarily used for logging/displaying the obstacle state.
ACTIVE_SNAP_ID = None 
SNAP_ARMED = False
...
def update_tiled_display():
    ...
    for obstacle_id, display_name, image_id, img in DETECTED_IMAGES:
        ...
        line2 = f"Obs ID = {obstacle_id}" # Display the log ID (e.g., ARROW)
...
def rpi_command_listener(log_sock):
    ...
    - SNAP_ID,{id}: Sets the active obstacle ID (now used just for state tracking)
        ...
        try:
            ...
            for line in data.splitlines():
                ...
                if line.startswith("SNAP_ID,"):
                    ...
                    # We use 'ARROW' as the ID if it's the specific target
                    if ACTIVE_SNAP_ID == "ARROW_DETECT":
                         print(f"\n🎯 SNAP ID RECEIVED: Ready for ARROW detection.")
                    else:
                         print(f"\n🎯 SNAP ID RECEIVED: Obstacle {ACTIVE_SNAP_ID}")

                elif line == "SNAPREADY":
                    SNAP_ARMED = True
                    # Set a generic ID if it hasn't been set by SNAP_ID,
                    if ACTIVE_SNAP_ID is None:
                         ACTIVE_SNAP_ID = "GENERIC"
                    print(f"🔫 SNAP ARMED for current task (Obs {ACTIVE_SNAP_ID})")
...
def main():
    ...
    # Removed last_sent_snap_id as it's no longer relevant for the single-command use case
    ...
    try:
        while True:
            ...
            # --- ARROW DETECTION LOGIC (Optimized) ---
            
            # This logic activates when RPi sends "SNAPREADY"
            if SNAP_ARMED: 
                ...
                if results and results[0].boxes and len(results[0].boxes) > 0:
                    ...
                    # Iterate through all detections to find the highest confidence arrow
                    for i, cls_id in enumerate(classes):
                        class_name = model.names[int(cls_id)]
                        
                        # Use the IMAGE_ID_MAP to get the standardized ID
                        _, image_id = get_display_info(class_name) 

                        # Look only for 'left' or 'right' arrows
                    ...
                    if best_direction:
                        ...
                        # Get annotated image with bounding boxes (for display/saving)
                        annotated = results[0].plot()
                        
                        # Log and print the detection
                        print(f"\n{'='*60}")
                        print(f"📸 ARROW DETECTION CAPTURED!")
                        print(f"   Detected: {display_name} -> {best_direction} Command")
                        print(f"   Confidence: {best_conf:.2%}")
                        print(f"{'='*60}\n")
                        
                        # Save images locally. Use ACTIVE_SNAP_ID if set, otherwise "ARROW"
                        log_id = ACTIVE_SNAP_ID if ACTIVE_SNAP_ID else "ARROW"
                        saved_path = save_detection_image(
                            log_id, target_name, display_name, image_id, raw_img, annotated
                        )
                        
                        # --- Send LEFT or RIGHT command back to RPi ---
                        final_result_string = best_direction
            ...
            # --- Status Indicator Correction ---
            status_id = ACTIVE_SNAP_ID if ACTIVE_SNAP_ID else "None"
            status_text = f"ARMED - Obs {status_id}" if SNAP_ARMED else "Monitoring"
